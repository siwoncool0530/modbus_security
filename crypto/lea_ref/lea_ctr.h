...
} // 바이트 단위 1씩 더해나감. 오버플로우(c)가 안나면 return, 오버플로우가 나면 다음 바이트로 넘어감. 16바이트 모두 오버플로우가 나면 종료.

static void ctr128_inc_aligned(unsigned char *counter) {
	unsigned int *data,c,n;
	const union { long one; char little; } is_endian = {1};

	if (is_endian.little) { // 1을 char로 읽었을 때 1이면 little endian, 0이면 big endian
		ctr128_inc(counter);
		return;
	}
	...
	do {
		...
		data[n] = c;
		if (c) return;
	} while (n); // 4번의 워드 연산으로 16바이트를 처리. 오버플로우(c)가 안나면 return, 오버플로우가 나면 다음 워드로 넘어감. 4워드 모두 오버플로우가 나면 종료.
}
