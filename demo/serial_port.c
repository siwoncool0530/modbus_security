...
int serial_port_open(serial_port_t *sp, const char *port_name, long baud, int read_interval_timeout_us)
{
    ...
    /* 알려진 미해결 문제: 새로 시작한 수신 프로세스의 "첫" serial_port_read() 호출이
       가끔(항상은 아님 -- 같은 바이너리로 10/10 성공한 적도, 0/10 실패한 적도 있음)
       결정론적으로 깨진 바이트를 받는 현상이 CM5 실 하드웨어에서 관찰됨(read() 개별
       syscall부터 이미 잘못된 바이트가 옴 -- 우리 코드가 데이터를 받은 뒤 망가뜨리는
       게 아님). 아래를 모두 테스트했지만 어느 것도 원인/해결책이 아님을 확인:
       T3.5 타이밍, ASYNC_LOW_LATENCY, RS485 완전 비활성화, open() 이후 안정화 지연
       (0~5000ms 실측), tcflush() 위치(재설정 전/후). 같은 바이너리가 실행마다 다르게
       동작하는 것으로 보아 결정론적 코드 버그가 아니라 간헐적인 전기적/커널 스케줄링
       레벨 문제로 추정 -- 오실로스코프/로직 애널라이저로 버스를 직접 봐야 확정 가능할
       듯. 재발하면 count>1로 재시도하거나 애플리케이션 레벨 재시도 로직을 고려할 것. */
